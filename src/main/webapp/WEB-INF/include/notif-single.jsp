...
<li class="notif-container" tipoNotifica="<%= n.getNotificationType() %>" usernameSrc="<%= n.getUserSrc().getUsername() %>" contenutoNotifica="<%= n.getNotificationMsg() %>">
    <div class="notifica">
        <div class="notif-wrapper <%=!n.isViewed() ? "da-leggere" : ""%>">
            ...
            <div><a href="<%= request.getContextPath() %>/userProfile?id=<%= n.getUserSrc().getId() %>&loggedId=<%= loggedUser.getId() %>" style="display: inline-block"><img src="../../images/propic.jpg" alt="immagine profilo" width="50" height="50"/></a></div>
            <div class="notif-content">
                <a href="<%= request.getContextPath() %>/userProfile?id=<%= n.getUserSrc().getId() %>&loggedId=<%= loggedUser.getId() %>"><p class="username">@<b class="username-bold"><%= n.getUserSrc().getUsername() %></b>:</p></a>
